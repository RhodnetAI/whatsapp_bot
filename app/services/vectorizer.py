...
async def store_product_service_vectors(
    item_id: str,
    name: str,
    text: str,
    created_at: str,
) -> None:
    """Embed a product/service description and upsert it into the same Qdrant
    collection the Information Agent already searches, linked back to the
    Supabase row via payload.document_id (reusing the existing indexed field
    so delete_document_vectors() works unchanged for re-embedding on edit)."""
    chunks = _split_text_into_chunks(text)
    if not chunks:
        return
...
async def store_scheduler_vectors(
    item_id: str,
    text: str,
    created_at: str,
) -> None:
    """Embed a scheduler entry and upsert it into Qdrant for RAG retrieval.
    Same pattern as store_product_service_vectors, linked via payload.document_id."""
    chunks = _split_text_into_chunks(text)
    if not chunks:
        return
